...
/**
 * Read-only division detail. Pure presentational — each app fetches
 * `division`, `parentSeason`, and `divisionTeams` via its own SDK
 * binding and passes them in. Both sa-web and org-admin-web mount
 * this so the canonical UI lives in one place.
 *
 * Sa-only add-ons (pending applications card, role-assignments panel)
 * go through the `pendingApplications` and `extras` slots; org-admin
 * omits them today.
 */
export function DivisionDetail({
  ...
  /** Optional bottom slot (e.g. sa-web's role-assignments). */
  extras?: ReactNode;
}) {
