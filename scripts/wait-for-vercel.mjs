...
async function latest(name, projectId) {
  const query = new URLSearchParams({ projectId, target: "production", teamId, limit: "1" });
...
while (Date.now() < deadline) {
  const status = await Promise.all(
    [...projects].map(async ([name, id]) => ({ name, deployment: await latest(name, id) }))
  );
  const pending = [];
  for (const { name, deployment } of status) {
    const state = deployment?.readyState ?? "MISSING";
    const current = deployment?.meta?.githubCommitSha === sha;
    if (current && (state === "ERROR" || state === "CANCELED")) {
      throw new Error(`${name} deployment for ${sha.slice(0, 7)} ended in ${state}`);
    }
    if (!current || state !== "READY") pending.push(`${name}:${current ? state : "WAITING"}`);
  }
  if (pending.length === 0) {
