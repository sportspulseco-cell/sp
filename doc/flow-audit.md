...
| 6 | **org-admin-web action mutations** ◐ | **Leagues + seasons + divisions + teams + comms broadcast done 2026-05-16** — five `/org-admin/...` write endpoints each delegate to the matching handler after per-org scope + `org_admin` role check. UI: `/leagues/new`, `/seasons/new`, `/divisions/new`, `/teams/new`, `/communications/compose`, each reachable from a "New …" / "Compose" CTA on the matching list page. The broadcast composer picks audiences (captains, team admins/coaches, players, all admins), writes one `org.broadcast` notification per resolved recipient (idempotent on broadcastId + personId) through the existing dispatcher path. **Still pending:** registrations form composer, finance write surfaces. | ~3 days remaining |
