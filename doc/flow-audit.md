...
| 6 | **org-admin-web action mutations** ◐ | **Leagues + seasons done 2026-05-16** — `OrgAdminLeaguesController` (`POST /org-admin/leagues`) and `OrgAdminSeasonsController` (`POST /org-admin/seasons`) each delegate to the existing CreateXHandler after a per-org scope + `org_admin` role check. UI: `/leagues/new` (name + sport + format) and `/seasons/new` (league dropdown + dates + optional registration window + roster lock). Both list pages now show a "New …" CTA in the header. **Still pending:** divisions, registrations + form mutations, finance write surfaces, team CRUD, communications composer. | ~2 weeks remaining |
