...
| 17 | **Org-admin extended actions** ◐ (assign captains ☑, kick off setup ☐, dispute resolution ☐) | **Captain assignment done 2026-05-15** — new `OrgAdminModule` with `OrgAdminTeamsController` at `/org-admin/teams/:teamId` (GET + assign-captain POST + revoke). Scope-guards on caller's org via `loadUserScope` + an explicit org_admin/super_admin role check. Syncs the legacy `teams.captain_user_id` column. UI: org-admin-web `/teams/[teamId]` detail page with an inline assign/revoke flow (paste user UUID). **Still pending:** org-admin kicks off org-setup, dispute resolution. | ~1 week remaining |
