...
| 6 | **org-admin-web action mutations** ◐ | **Leagues + seasons + divisions + teams done 2026-05-16** — four create endpoints on `/org-admin/...` each delegate to the matching CreateXHandler after a per-org scope + `org_admin` role check. UI: `/leagues/new`, `/seasons/new`, `/divisions/new`, `/teams/new`, each linked from a "New …" CTA in the matching list page header. Team create lands the user on the team-detail page so they can immediately assign a captain (via #17a). **Still pending:** registrations + form mutations, finance write surfaces, communications composer. | ~1 week remaining |
