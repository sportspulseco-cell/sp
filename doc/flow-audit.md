...
| 17 | **Org-admin extended actions** ◐ (assign captains ☑, dispute resolution ☑, kick off setup ☐) | **Captain assignment done 2026-05-15** — `OrgAdminTeamsController` at `/org-admin/teams/:teamId` (GET + assign-captain POST + revoke). Scope-guards on caller's org via `loadUserScope` + an org_admin/super_admin role check. Syncs the legacy `teams.captain_user_id` column. UI: org-admin-web `/teams/[teamId]` detail with inline assign/revoke. **Dispute resolution done 2026-05-16** — `OrgAdminRefundAssessmentsController` at `/org-admin/refund-assessments` (list scoped to caller's orgs + POST `/:id/resolve`). Decisions: refund (with capped amount), no_refund, void; each writes status + decisionNotes + resolvedAt. UI: org-admin-web `/disputes` page with status tabs (pending / refunded / declined / voided / all) and an inline adjudication form per row. **Still pending:** org-admin kicks off org-setup. | ~0.5 week remaining |
