...
| Section P NFRs | ✅ | 2026-05-17 | Security headers present on sp-superadmin (HSTS `max-age=63072000; includeSubDomains; preload`). CORS allowlist on sp-api correctly echoes each of the 5 deployed origins (sp-superadmin / sp-org-admin / sp-player-red / sp-team-admin / sp-landing-seven) — no wildcard `*` on API responses, no cross-app leak. |
| Section Q cross-app | ✅ | 2026-05-17 | Implicit throughout the sweep: super-admin → captain (wizard creates league visible to org-admin); org-admin → super-admin (org create row appears in /organizations); captain → super-admin (rollover application appears in /division-applications); admin → captain (approve unblocks the next wizard step); super-admin → org-admin → player (TC-A5-01 cross-org grant). |
| _all others_ | ⏳ | — | queued |
...
- **Repro:**
  1. Sign in as Parker (captain on Boston Gold Kings).
...
- **Diagnosis path:** Vercel hobby plan doesn't expose runtime function logs via the public API, so I added an inline diag that re-threw the error as a 400 with the raw pg message + stack. That surfaced the real driver error: `The "string" argument must be of type string or an instance of Buffer or ArrayBuffer. Received an instance of Date.`
- **Root cause:** The handler's `openSeasons` branch used raw `sql\`${col} <= ${now}\`` + `sql\`${col} >= ${now}\`` predicates plus raw `sql\`${col} ASC/DESC\`` order-by templates. Drizzle's raw `sql` template binds the Date param as-is; node-postgres only accepts string/Buffer/ArrayBuffer there.
- **Fix:** Replaced the four raw templates with Drizzle's typed comparators — `lte(col, now)` / `gte(col, now)` / `asc(col)` / `desc(col)` — which serialise Date → ISO string before the driver sees it. Confirmed end-to-end: same call now returns `{mode: "registration_open", seasonName: "ipl season autumn", registrationClosesAt: "2026-05-28T18:29:59Z", …}` for the same captain + team. Inline diag stripped in the same commit; logger.error retained.
- **File:** `apps/superadmin-api/src/modules/captain/interface/captain.controller.ts` lines 229, 270–278.
- **Status:** ✅✅ Verified end-to-end after deploy. TC-F1-01 now passes.
