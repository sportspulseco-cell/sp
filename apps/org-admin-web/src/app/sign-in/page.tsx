...
function SignInForm() {
  ...
  const [error, setError] = useState<string | null>(
    sp.get("error") === "wrong_role"
      ? "This account doesn't have org admin access. Try a different sign-in or contact your admin."
      : null
  );
      ...
      <form onSubmit={onSubmit} className="space-y-5 rounded-xl border border-border bg-surface-1 p-8">
        <div className="space-y-1">
          ...
          <p className="text-[12px] text-fg-muted">
            For org admin accounts only. Different role? Use the matching app.
          </p>
        </div>
        <Field label="Email">
          <Input
