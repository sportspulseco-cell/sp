...
/**
 * Captain approval-gate. Single inline surface — every state of the
 * registration flow renders on this same route:
 *   - ?submitted=<entryId>   → "Application submitted!" success view (mock 3)
 *   - pending_approval       → pending banner with withdraw + closes date
 *   - applied|accepted|confirmed → approved view: fee / threshold / Set up roster CTA
 *   - rejected (no active)   → denied view: pink reason box + apply-different
 *   - no active app          → list of open seasons with 3-stat cards
 */
export default async function CaptainRegisterPage({
  ...
  // Submitted-success view — entered via ?submitted=<entryId> after the
  // captain hits "Apply for this division". Only render while the entry
  // is still pending_approval; if admin has acted between submit and
  // refresh, fall through to the real outcome card so we never lie about
  // status.
  if (submitted) {
...
// ---------------------------------------------------------------------
// Mock 3 — Application submitted! success view
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Mock 1 — Pending application banner (highlighted)
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Mock 5 LEFT — Approved detail card
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Mock 5 RIGHT — Denied detail card
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Mock 1 — Open season card (3-stat row + arrow CTA)
// ---------------------------------------------------------------------
