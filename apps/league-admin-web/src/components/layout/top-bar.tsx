...
export function TopBar({
  ...
  return (
    <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-border bg-bg/80 px-6 backdrop-blur supports-[backdrop-filter]:bg-bg/60">
      <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-wide text-fg-muted">
        <ShieldCheck className="h-3 w-3" strokeWidth={1.75} />
        {isSuperAdmin ? (
          <span>super_admin · all leagues</span>
        ) : (
          <span>league_admin · {manageableCount} leagues</span>
        )}
      </div>
